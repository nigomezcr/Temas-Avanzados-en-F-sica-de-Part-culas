...
void HistPlotter(){
  ...
  cout << sEntries_ttbar << " " << endl;
    
  std::ostringstream sTop;
  sTop << setprecision(5) << h_singletop -> Integral(0, h_singletop -> GetNbinsX()+1);
  std::string single_Top = sTop.str();
  cout << single_Top << " " << endl;
    
  std::ostringstream Z;
  Z << setprecision(5) << h_zjets -> Integral(0, h_zjets -> GetNbinsX()+1);
  std::string Z_jets = Z.str();
  cout << Z_jets << " " << endl;
    
  std::ostringstream W;
  W << setprecision(5) << h_wjets -> Integral(0, h_wjets -> GetNbinsX()+1);
  std::string W_jets = W.str();
  cout << W_jets << " " << endl;

  std::ostringstream DY;
  DY << setprecision(5) << h_drellyan -> Integral(0, h_drellyan -> GetNbinsX()+1);
  std::string Drell_Yan = DY.str();
  cout << Drell_Yan << " " << endl;


  std::ostringstream db;
  db << setprecision(5) << h_db -> Integral(0, h_db -> GetNbinsX()+1);
